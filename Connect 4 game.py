...
def gameBoardAndUpdating(turnNumber, x, y):
    ...
    if tempRow>=0 and turnNumber%2==0: 
        ...
        print("Placing RED in", tempRow, tempCol, "AND inRows=", inRows, flush=True)
    ...
    for row in range(len(gameBoard)): 
        ...
        for col in range(len(gameBoard[0])):
            ...
            if gameBoard[row][col]== Draw.YELLOW:
                currYellow+=1                print("Current Yellow is ", currYellow)
                currRed = 0 
                #update original bestRuns if a longer bestRun is found, and
                #there is an empty space next to it
                if currYellow>bestHorizontalYellow[0] and col!=6 and inRows[col+1]<row: #***changed to be not equal because want to know if there's an empty space there
                    bestHorizontalYellow[0] = currYellow
                    bestHorizontalYellow[1] = row 
                    bestHorizontalYellow[2] = col +1 #exactly where the piece should be placed                    print("bestHorizontalYellow: ", bestHorizontalYellow)
                #if no empty space after the "string" of yellows, check before it
                elif currYellow>bestHorizontalYellow[0] and col-currYellow!=0 and inRows[col-currYellow]<row: ##also changed here from ==row to <row
                    bestHorizontalYellow[0] = currYellow
                    bestHorizontalYellow[1] = row
            ...
            #current red list and reset yellow's current best run to zero
                ...
                if currRed>bestHorizontalRed[0] and col!= 6 and inRows[col+1]<row: #**
                ...
                elif currRed>bestHorizontalRed[0] and col-currRed!= 6 and inRows[col-currRed]<row:
                    bestHorizontalRed[0] = currRed
                    bestHorizontalRed[1] = row
    ...
    for col in range(len(gameBoard[0])):
        ...
        for row in range(len(gameBoard)):
            ...
            if gameBoard[row][col]== Draw.YELLOW:
                ...
                if currYellow>bestVerticalYellow[0] and row!=0 and inRows[col]>=0: 
...
def diagonalREDPiecesInRow(bestDiag1Red, bestDiag2Red, \
    ...
    if bestDiag1Red[0]>bestDiag2Red[0] and \       bestDiag1Red[0]>bestDiag3Red[0] and \       bestDiag1Red[0]>bestDiag4Red[0]:
        ...
        bestRedDiag = bestDiag4Red    ###Possible place to reduce lines of code here
    return bestRedDiag
...
def bestRunsInformation():
    ...
    bestYellowDiagonal = diagonalYELLOWPiecesInRow(bestDiag1Yellow, \
                                                   ...
                                                   bestDiag3Yellow, \
                                                   bestDiag4Yellow)
        print("bestVerticalRed: ", bestVerticalRed)    print("bestHorizontalRed: ", bestHorizontalRed)    print("bestRedDiagonal: ", bestRedDiagonal)
    ...
    theBestYellow = yellowBest(bestVerticalYellow, bestHorizontalYellow, \
                               bestRedDiagonal)
        print("the best red is: ", theBestRed)    print("the best yellow is: ", theBestYellow)
...
def compStrategy(totalRedBestRun, totalYellowBestRun):
    ...
    #bestRun of 2 yellows
        ...
        col = totalYellowBestRun[2]
        gameBoard[row][col] = Draw.YELLOW
        inRows[col]= inRows[col] - 1        print("2-Y Placing YELLOW in", row, col, "AND inRows is now=", inRows, flush=True)        
    #if no bestRun of 2 yellows, check for bestRun of 2 reds (player's pieces)
    elif totalRedBestRun[0] == 2:
        row = totalRedBestRun[1]-1   #***
        col = totalRedBestRun[2]        print("gb: ", gameBoard)
        gameBoard[row][col] = Draw.YELLOW  ####NOTE: take this out and put bellow to reduce redundancy
        inRows[col]= inRows[col] - 1
        print("2-R Placing YELLOW in", row, col, "AND inRows is now=", inRows, flush=True)
